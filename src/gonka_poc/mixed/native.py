...
def attach_native_poc(model: nn.Module, layers: list, embed_owner, max_tokens: int,
    ...
    for wrapper in layers:
        ...
        # Selection override (modular kernel path): the authoritative seam.
        # The gate-logit forcing above stays as cover for monolithic kernels
        # that fuse routing into the expert kernel.
        _router = getattr(moe.experts, "router", None)
        if _router is not None:
            _install_poc_select_patch(_router, PoCSelectOverride(
                route_base, state.route_step, n_exp, top_k, state.mask))
